...
@DatabaseView("SELECT i.id, i.inspector_id, i.inspection_type_id, i.inspection_status_id, i.reinspect, i.community, i.address, i.inspection_type, i.notes, i.job_number, i.require_risk_assessment, i.inspection_date, i.is_complete, i.is_failed, i.is_uploaded, SUM(CASE WHEN id.is_uploaded = 1 THEN 1 ELSE 0 END) AS num_uploaded, COUNT(id.id) AS num_total, route_sheet_order " +
