...
@Dao
public interface Ekotrope_Data_DAO { //ToDo: Ekotrope data needs to be used
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    void insert(Ekotrope_Data_Table inspection);

    @Query("DELETE FROM ekotrope_data_table")
    void deleteAll();

    @Query("DELETE FROM ekotrope_data_table WHERE id = :id")
    void delete(int id);

    @Query("UPDATE ekotrope_data_table SET " +
    ...
    @Query("SELECT * FROM ekotrope_data_table WHERE id = :inspection_id")
    LiveData<Ekotrope_Data_Table> getInspection(int inspection_id);

    @Query("SELECT * FROM ekotrope_data_table WHERE id = :inspection_id")
    Ekotrope_Data_Table getInspectionSync(int inspection_id);

    @Query("SELECT id FROM ekotrope_data_table WHERE inspector_id = :inspector_id")
    List<Integer> getAllInspectionIds(int inspector_id);

    @Query("SELECT reinspect FROM ekotrope_data_table WHERE id = :inspection_id")
    boolean getReinspect(int inspection_id);

    @Query("UPDATE ekotrope_data_table SET is_complete = 1, is_failed = 0, end_time = :end_time WHERE id = :inspection_id")
    void completeInspection(OffsetDateTime end_time, int inspection_id);

    @Query("UPDATE ekotrope_data_table SET is_failed = 1 WHERE id = :inspection_id")
    void markInspectionFailed(int inspection_id);

    @Query("UPDATE ekotrope_data_table SET is_uploaded = 1 WHERE id = :inspection_id")
    void uploadInspection(int inspection_id);

    @Query("UPDATE ekotrope_data_table SET route_sheet_order = :new_order WHERE id = :inspection_id")
    void updateRouteSheetIndex(int inspection_id, int new_order);

    @Query("UPDATE ekotrope_data_table SET start_time = :start_time WHERE id = :inspection_id")
    void startInspection(OffsetDateTime start_time, int inspection_id);

    @Query("SELECT COUNT() FROM ekotrope_data_table WHERE is_complete = 0")
    int getIndividualRemainingInspections();

    @Query("UPDATE ekotrope_data_table SET trainee_id = :trainee_id WHERE id = :inspection_id")
