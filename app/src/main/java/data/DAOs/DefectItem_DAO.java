...
public interface DefectItem_DAO {
    ...
    @Query("SELECT d.* FROM defect_item_table d INNER JOIN defect_item_x_inspection_type x ON x.defect_item_id = d.id WHERE x.inspection_type_id = :inspection_type_id ORDER BY d.defect_category_id, d.item_number ASC")
